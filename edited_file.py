...
async def translate_hindi_to_english(hindi_text):
    translator = Translator()
    translation = await translator.translate(hindi_text, src='en', dest='hi')
    return translation.text


def on_translate():
    hindi_text = text_input.get("1.0", "end-1c")  
    if hindi_text.lower() == "exit":
        root.quit()
    else:
        english_translation = asyncio.run(translate_hindi_to_english(hindi_text))
        text_output.delete("1.0", "end")  
        text_output.insert(tk.END, english_translation)  

def on_exit():
    messagebox.showinfo("Goodbye", "Goodbye aaj ke liye bas itna hi aapki tarah ham bhi mayus hen !")
    root.quit()
...
label = tk.Label(root, text="Enter Hindi text below to translate to English:", font=("Arial", 12), bg="#f0f0f0")
label.pack(pady=10)
...
exit_button = tk.Button(root, text="Exit ", font=("Arial", 12), bg="#f44336", fg="black", command=on_exit)
...
def on_exit():
    messagebox.showinfo("Goodbye", "Goodbye! Aaj ke liye bas itna hi Milte hen fir kabhi .")
    root.quit()
...
label = tk.Label(root, text="Enter text to translate:", font=("Arial", 12), bg="#f0f0f0")
label.pack(pady=5)
